...
contract('Requiem Votes Register: Base', function (accounts) {
  ...
  beforeEach(async function () {
    ...
    this.register = await VotingRegister.new({ from: holder })
  ...
  it('domain separator', async function () {
    expect(
      ...
      await domainSeparator('Requiem Pool Voting Register', version, this.chainId, this.register.address),
    );
  });
  ...
  describe('transfers', function () {
    beforeEach(async function () {
      await this.token1.mint(holder, supply);
      let x = await this.register.isRegistered(this.token1.address)
      console.log(x)
    });
    ...
    describe('numCheckpoints', function () {
      ...
      it('does not add more than one checkpoint in a block', async function () {
        ...
        const [t1, t2, t3] = await batchInBlock([
          () => this.register.delegate(other1, this.token1.address, { from: recipient, gas: 100000 }),
          () => this.token1.transfer(other2, 10, { from: recipient, gas: 100000 }),
          () => this.token1.transfer(other2, 10, { from: recipient, gas: 100000 }),
        ]);
        expect(await this.register.numCheckpoints(other1, this.token1.address)).to.be.bignumber.equal('1');
